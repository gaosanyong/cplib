...
class SegTree {
     ...
     * @param {number} hi  - the higher end of the array representing the tree.
     */
    update(i: number, val: number, k=0, lo=0, hi=this.n): void {
...
class LazySegTreeMin {
     ...
     * @param {number} hi  - the higher end of the array representing the tree.
     */
    update(qlo: number, qhi: number, delta: number, k=0, lo=0, hi=this.n): void {
        if (this.lazy[k]) {
        ...
        if (lo < hi && qlo < hi && lo < qhi) {
            if (qlo <= lo && hi <= qhi) {
                ...
                this.update(qlo, qhi, delta, 2*k+1, mid, hi);
                this.tree[k] = Math.min(this.tree[2*k+1], this.tree[2*k+2]);
            }
        }
    }
